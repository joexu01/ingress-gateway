...
import (
	...
	"flag"
	"fmt"
	sLog "github.com/joexu01/ingress-gateway/log"
	"io/ioutil"
	"log"
	"math/rand"
	"net"
...
func addTrace2Header(request *http.Request, trace *TraceContext) *http.Request {
	...
	cSpanId := NewSpanId()
	if traceId != "" {
		request.Header.Set("frame-header-rid", traceId)
	}
	if cSpanId != "" {
		request.Header.Set("frame-header-span-id", cSpanId)
	}
...
func GetMd5Hash(text string) string {
	hashMan := md5.New()
	hashMan.Write([]byte(text))
	return hex.EncodeToString(hashMan.Sum(nil))
}
