...
func SetupLogInstanceWithConf(lc Config, logger *Logger) (err error) {
	if lc.FW.On {
		if len(lc.FW.LogPath) > 0 {
			w := NewFileWriter()
			w.SetFileName(lc.FW.LogPath)
			_ = w.SetPathPattern(lc.FW.RotateLogPath)
		...
		if len(lc.FW.WfLogPath) > 0 {
			...
			_ = wfw.SetPathPattern(lc.FW.RotateWfLogPath)
...
func SetupDefaultLogWithConf(lc Config) (err error) {
	defaultLoggerInit()
